#' @title Upload Data to BigQuery (Parquet Optimized)
#' @description Authentifiziert sich via JSON-Service-Account-Key, extrahiert automatisch die Projekt-ID und lädt ein Dataframe extrem schnell via Parquet in BigQuery hoch.
...
upload_to_bigquery <- function(data,
  ...
  # 5. Temporäre Parquet-Datei erstellen und On-Exit-Löschung garantieren
  temp_file <- tempfile(fileext = ".parquet")
  on.exit(unlink(temp_file), add = TRUE)

  message("Schreibe temporäre Parquet-Datei für den Upload...")
  arrow::write_parquet(data, temp_file)

  # 6. Upload der Daten via Parquet
  tryCatch(
    {
      message("Starte High-Speed Daten-Upload nach BigQuery (", nrow(data), " Zeilen)...")

      bigrquery::bq_table_upload(
        x = tb,
        values = temp_file, # Wir übergeben den Dateipfad zur Parquet-Datei
        source_format = "PARQUET", # ZWINGEND FÜR PARQUET!
